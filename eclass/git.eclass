...
# @ECLASS-VARIABLE: EGIT_HAS_WORKDIR
# @DESCRIPTION:
# Set this to non-empty value to update working tree
: ${EGIT_HAS_WORKDIR:=}
...
git_fetch() {
	...
	[[ -z ${EGIT_HAS_SUBMODULES} ]] && [[ -z ${EGIT_HAS_WORKDIR} ]] && export GIT_DIR
	...
	if [[ -n ${EGIT_HAS_SUBMODULES} || -n ${EGIT_HAS_WORKDIR} ]] && [[ -d ${GIT_DIR} && ! -d ${GIT_DIR}/.git ]]; then
		rm -rf "${GIT_DIR}"
		einfo "The ${EGIT_CLONE_DIR} was bare copy. Refetching."
	fi
	if [[ -z ${EGIT_HAS_SUBMODULES} && -z ${EGIT_HAS_WORKDIR} ]] && [[ -d ${GIT_DIR} && -d ${GIT_DIR}/.git ]]; then
		rm -rf "${GIT_DIR}"
		einfo "The ${EGIT_CLONE_DIR} was not a bare copy. Refetching."
	fi

	if [[ -n ${EGIT_HAS_SUBMODULES} ]] || [[ -n ${EGIT_HAS_WORKDIR} ]]; then
